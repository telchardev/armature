...
/// loading state (owning feature [FeatureStatus.pending]). Invoked on
/// every build where the loader path is active; typical implementations
/// return a `const` placeholder.
...
/// renderer-wide fallback while the feature is still
...
class SlotDescriptor {
  /// Widget rendered once the owning feature is [FeatureStatus.active]
  /// and this handler wins selection.
  final Widget widget;

  /// Optional loader shown while the owning feature is
  /// [FeatureStatus.pending]. When `null`, the renderer's global
  /// `loaderBuilder` is used (or an empty space if that's also absent).
